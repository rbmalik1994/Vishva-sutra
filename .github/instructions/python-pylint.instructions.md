---
applyTo: "**/*.py"
description: Guidelines for configuring and using Pylint effectively.
---
...
## 1. Configuration lives in `pyproject.toml`

Always use `pyproject.toml` for Pylint configuration. This ensures consistency across all environments and developers.

**Action**: Generate a baseline config and merge it into your existing
`pyproject.toml` (do not overwrite an existing file).
```bash
pylint --generate-toml-config > pylint.generated.toml
```

Copy the `[tool.pylint.*]` sections you want into `pyproject.toml`.

### 1.1. Silence the Noise, Enable What Matters

Pylint is noisy by default. If the project already has an allowlist, extend it.
Otherwise start by disabling everything and selectively enabling relevant
categories. Focus on `convention`, `refactor`, `warning`, and `error`.
...
Run Pylint automatically before commits. If you use multiple linters, keep the
fast/auto-fix tools first (formatter/import sorting), and run Pylint after.
...
## 5. Pylint in a modern Python stack

Pylint is one piece of a comprehensive linting strategy.

*   **Ruff** (optional): Fast style checks, basic errors, import sorting.
*   **Black/Ruff format** (optional): Code formatting.
*   **MyPy/Pyright** (optional): Static type checking.
*   **Pylint**: Deeper semantic analysis, code smells, architectural issues.
