...
You WILL:
- Include a short metadata header at the top of each file with fields: Prompt-Name, Project, Author, Created-At (ISO8601), Sources, Version, Status (draft|reviewed|final).
- Use kebab-case or snake_case for prompt_name and project_name; replace spaces with hyphens and remove special characters.
- Put drafts in .project/plans/drafts/ and final/approved plans in .project/plans/final/ (optional subfolders allowed).
- Commit the plan file to the same feature branch used for prompt changes, or store in .project/plans/drafts/ when under review.
- NEVER include secrets, credentials, or personally-identifying information in plan files.
- Update Version and Status when making revisions and keep prior versions (append incremental version numbers or preserve timestamped copies).
